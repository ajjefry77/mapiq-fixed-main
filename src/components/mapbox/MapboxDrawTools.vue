<template>
  <div class="absolute top-[calc(var(--top)+137px)] left-1 z-50">
    <div @click.stop class="flex flex-col rounded shadow-md p-2 gap-2">
      <button @click="toggleMeasure" title="اندازه گیری" style="margin: 0"
...
function clearTempLayers() {
  tempLayerIds.forEach(id => { if (props.map.getLayer(id)) props.map.removeLayer(id); });
  tempLayerIds = [];
  if (tempSourceId && props.map.getSource(tempSourceId)) { props.map.removeSource(tempSourceId); tempSourceId = null; }
}

function cleanupHandlers() {
  if (mouseMoveHandler) { props.map.off('mousemove', mouseMoveHandler); mouseMoveHandler = null; }
  if (clickHandler) { props.map.off('click', clickHandler); clickHandler = null; }
  if (dblClickHandler) { props.map.off('dblclick', dblClickHandler); dblClickHandler = null; }
  if (rightClickHandler) { props.map.off('contextmenu', rightClickHandler); rightClickHandler = null; }
...
function startDrawing() {
  ...
  if (drawMode.value === 'multi_point') {
    ...
    map.addLayer({ id: tempSourceId + '-points', type: 'circle', source: tempSourceId,
    ...
    tempSourceId = 'temp-' + crypto.randomUUID();
    map.addSource(tempSourceId, { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
    map.addLayer({ id: tempSourceId + '-line', type: 'line', source: tempSourceId,
      paint: { 'line-color': color.value, 'line-width': 3 } });
    map.addLayer({ id: tempSourceId + '-points', type: 'circle', source: tempSourceId,
      filter: ['==', '$type', 'Point'],
      paint: { 'circle-radius': 5, 'circle-color': '#ffff00', 'circle-stroke-color': '#000000', 'circle-stroke-width': 2 } });
    ...
    tempSourceId = 'temp-' + crypto.randomUUID();
    map.addSource(tempSourceId, { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
    map.addLayer({ id: tempSourceId + '-fill', type: 'fill', source: tempSourceId,
      paint: { 'fill-color': color.value, 'fill-opacity': 0.4 } });
    map.addLayer({ id: tempSourceId + '-outline', type: 'line', source: tempSourceId,
      paint: { 'line-color': color.value, 'line-width': 3 } });
    map.addLayer({ id: tempSourceId + '-points', type: 'circle', source: tempSourceId,
      filter: ['==', '$type', 'Point'],
      paint: { 'circle-radius': 6, 'circle-color': '#ffff00', 'circle-stroke-color': '#000000', 'circle-stroke-width': 2 } });
    ...
    clickHandler = (e) => {
      if (!center) {
        ...
        cleanupHandlers();
        finishDrawing("circle", { center: { lng: center[0], lat: center[1] }, radius });
        showForm.value = true;
      }
    };
    ...
    mouseMoveHandler = (e) => {
      if (!center) return;
      const dx = (e.lngLat.lng - center[0]) * 111319.9 * Math.cos(center[1] * Math.PI / 180);
      ...
      for (let i = 0; i <= 64; i++) {
        ...
        const rLng = center[0] + (radius / (111319.9 * Math.cos(center[1] * Math.PI / 180))) * Math.cos(angle);
...
function updateTempGeoJSONWithPreview(pts) {
  ...
  const pointFeatures = pts.map((p, i) => ({
    type: 'Feature', geometry: { type: 'Point', coordinates: [p.lng, p.lat] }, properties: { isPreview: i === pts.length - 1 }
  }));
...
function finishDrawing(draw, pos) {
  ...
  if (draw === "circle") {
    shape.value = { type: "circle", center: pos.center, radius, color: color.value, show: true };
  } else if (draw === "multi_point") {
    shape.value = {
      type: "multi_point",
      positions: pos.map(p => ({ lon: p.lng, lat: p.lat, height: 0, color: p.color })),
      width: 5, color: color.value, show: true
    };
  } else if (draw === "polyline") {
    shape.value = {
      type: draw,
      positions: pos.map(p => ({ lon: p.lng, lat: p.lat, height: 0 })),
      width: 3, color: color.value, show: true
    };
  } else if (draw === "polygon") {
    const coords = pos.map(p => ({ lon: p.lng, lat: p.lat, height: 0 }));
    coords.push(coords[0]);
    shape.value = { type: "polygon", positions: coords, color: color.value, outlineColor: color.value, show: true };
  }
...
function startMeasure() {
  ...
  map.addLayer({ id: tempId + '-line', type: 'line', source: tempId, paint: { 'line-color': '#00ff00', 'line-width': 3 } });
  map.addLayer({ id: tempId + '-points', type: 'circle', source: tempId, filter: ['==', '$type', 'Point'],
    paint: { 'circle-radius': 8, 'circle-color': '#ff0000' } });
  map.addLayer({ id: tempId + '-labels', type: 'symbol', source: tempId, filter: ['has', 'distance'],
    layout: { 'text-field': ['get', 'distance'], 'text-size': 14, 'text-offset': [0, -1.5] },
...
const savePin = async () => {
  ...
  if (attch_file.value) { pin.filename = attch_file.value.name; pin.file = attch_file.value; }
