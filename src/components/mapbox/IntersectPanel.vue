<template>
  ...
  >
    ...
    <!-- حالت ۲: در حال تحلیل -->
    <div v-else-if="analyzing" class="text-zinc-400 text-center py-3">
      در حال تحلیل...
    </div>
    ...
    <template v-else-if="overlapSourceLabel">
      ...
      >
        <div
          ...
          class="p-2"
        >
          <div class="font-semibold text-zinc-100 text-xs mb-1 flex items-center gap-1">
            <i class="fas fa-vector-square text-orange-500"></i>
            {{ grp.layerName }}
            <span class="text-[10px] font-normal text-zinc-500" dir="ltr">
          ...
          <ul class="pr-2 space-y-1">
            <li
              ...
              :key="'i-' + ii"
              class="text-[11px] text-zinc-400 bg-neutral-950 rounded px-2 py-1.5 leading-relaxed"
            >
              همپوشانی با
              <span class="font-medium text-zinc-100">«{{ item.otherName }}»</span>:
      ...
      >
        <div
          ...
          class="text-center text-zinc-500 py-3 text-xs"
        >
        ...
        <div
          ...
          class="p-2 text-xs"
        >
          <div class="font-semibold text-zinc-100">{{ r.pinName }}</div>

          <div v-if="r.kind === 'point'" class="text-zinc-400 mt-0.5" dir="ltr">
            نقطه — {{ r.lat.toFixed(6) }}, {{ r.lon.toFixed(6) }} | UTM:
            {{ r.utmX.toFixed(1) }}, {{ r.utmY.toFixed(1) }} (Z{{ r.utmZone }})
          </div>

          <div v-else-if="r.kind === 'line'" class="text-zinc-400 mt-0.5">
            خط —
            <span dir="ltr">
          ...
          <div v-else-if="r.kind === 'polygon'" class="text-zinc-400 mt-0.5">
    ...
    <template v-else>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="border rounded p-2 mt-1">
          ...
          <div
            v-if="!polygonPinOptions.length"
            class="text-center text-zinc-500 text-xs py-2"
          >
          ...
          >
            ...
            >
              ...
              />
              <span class="flex-1 truncate text-zinc-400" :title="opt.name">
                {{ opt.name }}
              </span>
