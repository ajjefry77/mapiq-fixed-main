<template>
  <div class="page">
    ...
    <div v-else-if="!pagedUsers.length" class="empty-state card">هنوز کاربری وجود ندارد.</div>
    <div v-else>
      <!-- Desktop table -->
      ...
      <div v-if="totalPages > 1" class="pagination-bar card">
        <div class="pagination-info">
          نمایش <strong>{{ (page - 1) * perPage + 1 }}</strong> تا <strong>{{ Math.min(page * perPage, users.length) }}</strong> از <strong>{{ users.length }}</strong> کاربر
        </div>
...
async function deleteUser(u) {
  ...
  try { await axios.delete(SERVER + `/api/users/${u.id}`); await loadUsers(); } catch (e) { console.error(e); }
}
