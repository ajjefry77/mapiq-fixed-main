...
function utmToLatLng({ x, y, zone, northern = true }) {
  ...
  let R1 =
      (a * (1 - eccSquared)) /
      Math.pow(1 - eccSquared * Math.sin(phi1Rad) ** 2, 1.5);
  let D = xAdj / (N1 * k0);
  ...
  let lat =
      ...
      R1 *
      ((D * D) / 2 -
          ((5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * eccSquared) *
              D ** 4) /
          24 +
          ((61 + 90 * T1 + 298 * C1 + 45 * T1 ** 2 - 252 * eccSquared - 3 * C1 ** 2) *
              D ** 6) /
          720);
  ...
  let lng =
      ((D -
          ((1 + 2 * T1 + C1) * D ** 3) / 6 +
          ((5 - 2 * C1 + 28 * T1 - 3 * C1 ** 2 + 8 * eccSquared + 24 * T1 ** 2) *
              D ** 5) /
          120) /
          Math.cos(phi1Rad)) +
...
function goToLocation_old() {
  if (!props.viewer || !lat_x.value || !lng_y.value) return;

  const [lat, lon] = parts;
  const destination = Cesium.Cartesian3.fromDegrees(lng_y, lat_x, 1500); // ارتفاع پیش‌فرض
  props.viewer.camera.flyTo({
    destination,
    duration: 2,
  });
